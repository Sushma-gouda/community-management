    ...
    <DashboardLayout role="Security" items={securityNav}>
      <div className="space-y-6 animate-fade-up">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            label="Guests"
            value={String(visitors.filter((v) => v.purpose === "Guest").length)}
            icon={UserCheck}
            tone="accent"
          />
          <StatCard
            label="Service"
            value={String(visitors.filter((v) => v.purpose === "Service").length)}
            icon={UserCheck}
            tone="warning"
          />
          ...
          <div className="relative w-full max-w-md rounded-2xl glass-strong shadow-elegant p-6 animate-scale-in">
            ...
            <div className="space-y-2.5">
              {[
                ...
                ...(selected.vehicle
                  ? [{ icon: Building2, label: "Vehicle", value: selected.vehicle }]
                  : []),
              ].map((d) => (
                <div
                  key={d.label}
            ...
            <div className="mt-6 flex items-center gap-2">
              <button
                onClick={() => setSelected(null)}
                className="flex-1 h-10 rounded-lg text-sm font-medium hover:bg-foreground/5"
              >
                Close
              </button>
              <button
