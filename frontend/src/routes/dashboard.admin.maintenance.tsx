...
const schedule = [
  {
    date: "Tomorrow",
    time: "09:00",
    title: "Lift B1 quarterly service",
    vendor: "Otis Care",
  ...
  },
  {
    date: "May 18",
    time: "10:30",
    title: "Water tank cleaning - all blocks",
    vendor: "Aquaclean",
    ...
    <DashboardLayout role="Admin" items={adminNav}>
      <div className="space-y-6 animate-fade-up">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatCard
            label="Healthy"
            value={String(assets.filter((a) => a.status === "Healthy").length)}
            icon={CheckCircle2}
            tone="success"
          />
          <StatCard
            label="Due Soon"
            value={String(assets.filter((a) => a.status === "Due Soon").length)}
            icon={Calendar}
            tone="warning"
          />
          <StatCard
            label="Overdue"
            value={String(assets.filter((a) => a.status === "Overdue").length)}
            icon={AlertCircle}
            tone="accent"
          />
        </div>
                ...
                {assets.map((a) => (
                  <div key={a.id} className="rounded-xl glass p-4 hover:shadow-card transition">
                    ...
                    <div className="mt-3 flex items-center justify-between text-[11px] text-muted-foreground">
                      ...
                      <span>
                        Next:{" "}
                        <span className="font-medium text-foreground/80">{a.nextService}</span>
                      </span>
                    </div>
                  </div>
                ))}
