...
export function AuthLayout({
  title, subtitle, children, footer, role, onRole,
}: {
  title: string; subtitle: string; children: ReactNode; footer: ReactNode;
  role: Role; onRole: (r: Role) => void;
}) {
  return (
        ...
        <div className="relative z-10 h-full flex flex-col p-12 text-white">
          <Link to="/" className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] uppercase tracking-[0.2em] text-white/60">Apartment OS</div>
            </div>
          </Link>
          <div className="mt-auto max-w-md">
            ...
            <p className="mt-4 text-white/75">
              Trusted by 120+ communities to manage residents, visitors, billing and security in one place.
            </p>
            <div className="mt-8 flex gap-6 text-white/80 text-sm">
              <div><div className="text-2xl font-semibold text-white">10k+</div>Residents</div>
              <div><div className="text-2xl font-semibold text-white">99.99%</div>Uptime</div>
              <div><div className="text-2xl font-semibold text-white">4.9★</div>App rating</div>
            </div>
          </div>
        </div>
      </div>
...
export function Field({
  label, type = "text", required, children, hint,
}: { label: string; type?: string; required?: boolean; children?: ReactNode; hint?: string }) {
  return (
...
export function PasswordField({ label = "Password", required }: { label?: string; required?: boolean }) {
