...
export function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <aside
        className={cn(
          "fixed lg:sticky top-0 left-0 z-40 h-screen w-72 shrink-0 transform transition-transform duration-300 lg:translate-x-0",
          open ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="h-full m-3 lg:m-4 rounded-2xl glass-strong shadow-card flex flex-col">
          ...
          <div className="p-3 border-t border-border">
            <div className="flex items-center gap-3 p-2 rounded-xl">
              <div className="h-9 w-9 rounded-full bg-[image:var(--gradient-primary)] grid place-items-center text-white text-sm font-semibold">
                {role[0]}
              </div>
      ...
      {/* Main area */}
      <div className="lg:pl-0 lg:flex">
        <div className="hidden lg:block w-72 shrink-0" /> {/* spacer matches absolute sidebar */}
        <div className="flex-1 min-w-0">
          <header className="sticky top-0 z-20 px-3 lg:px-4 pt-3 lg:pt-4">
            <div className="rounded-2xl glass-strong shadow-card px-4 sm:px-5 py-3 flex items-center gap-3">
              <button onClick={() => setOpen(true)} className="lg:hidden p-2 rounded-md hover:bg-foreground/5">
                <Menu className="h-5 w-5" />
              </button>
              <div className="relative flex-1 max-w-md hidden sm:block">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <input
                  placeholder="Search residents, complaints, visitors…"
              ...
              <div className="ml-auto flex items-center gap-1.5">
                <button onClick={toggle} className="grid place-items-center h-9 w-9 rounded-lg hover:bg-foreground/5 text-foreground/80">
                ...
                <button className="hidden sm:flex items-center gap-2 h-9 pl-1.5 pr-3 rounded-lg hover:bg-foreground/5">
                  <div className="h-6 w-6 rounded-full bg-[image:var(--gradient-primary)] grid place-items-center text-white text-[11px] font-semibold">{role[0]}</div>
          ...
          <main className="px-3 lg:px-4 py-5 lg:py-6">{children}</main>
        </div>
      </div>
    </div>
  );
...
}: { label: string; value: string; change?: string; icon: React.ComponentType<{ className?: string }>; tone?: "primary" | "success" | "warning" | "accent" }) {
...
export function Badge({ children, tone = "primary" }: { children: ReactNode; tone?: "primary" | "success" | "warning" | "danger" | "muted" | "accent" }) {
  const toneMap: Record<string, string> = {
    primary: "bg-primary/10 text-primary",
    success: "bg-[color:var(--success)]/15 text-[color:var(--success)]",
    warning: "bg-[color:var(--warning)]/15 text-[color:var(--warning)]",
  ...
  return <span className={cn("inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium", toneMap[tone])}>{children}</span>;
}
