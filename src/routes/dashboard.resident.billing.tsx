...
import { FileText, CreditCard, X, CheckCircle2, Clock, AlertTriangle, Download } from "lucide-react";
...
type Bill = {
  id: string; month: string; amount: number; due: string;
  status: BillStatus; type: string; paidOn?: string;
};

const bills: Bill[] = [
  { id: "INV-9821", month: "May 2026", amount: 4500, due: "May 30, 2026", status: "Pending", type: "Maintenance" },
  { id: "INV-9810", month: "April 2026", amount: 4500, due: "Apr 30, 2026", status: "Paid", type: "Maintenance", paidOn: "Apr 28, 2026" },
  { id: "INV-9799", month: "March 2026", amount: 4500, due: "Mar 30, 2026", status: "Paid", type: "Maintenance", paidOn: "Mar 27, 2026" },
  { id: "INV-9788", month: "February 2026", amount: 4500, due: "Feb 28, 2026", status: "Paid", type: "Maintenance", paidOn: "Feb 25, 2026" },
  { id: "INV-9777", month: "January 2026", amount: 4500, due: "Jan 31, 2026", status: "Paid", type: "Maintenance", paidOn: "Jan 29, 2026" },
  { id: "INV-9766", month: "December 2025", amount: 5200, due: "Dec 31, 2025", status: "Paid", type: "Maintenance + Repair", paidOn: "Dec 28, 2025" },
  { id: "INV-9755", month: "November 2025", amount: 4500, due: "Nov 30, 2025", status: "Paid", type: "Maintenance", paidOn: "Nov 27, 2025" },
];
...
function ResidentBilling() {
  ...
  const tone = (s: BillStatus) => s === "Paid" ? "success" : s === "Pending" ? "warning" : "danger";
    ...
    <DashboardLayout role="Resident" items={residentNav}>
      <div className="space-y-6 animate-fade-up">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="rounded-2xl glass shadow-card p-5 bg-gradient-to-br from-[color:var(--warning)]/20 to-[color:var(--warning)]/5">
            ...
            <div className="text-[11px] text-muted-foreground mt-1">{pending.length} bill(s) due</div>
          </div>
          <div className="rounded-2xl glass shadow-card p-5 bg-gradient-to-br from-[color:var(--success)]/20 to-[color:var(--success)]/5">
            ...
            <div className="mt-2 text-2xl font-semibold text-[color:var(--success)]">₹{totalPaid.toLocaleString()}</div>
            <div className="text-[11px] text-muted-foreground mt-1">{bills.filter((b) => b.status === "Paid").length} bills paid</div>
          </div>
        ...
        {pending.length > 0 && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Pending Bills</h2>
            {pending.map((b) => (
              <div
                key={b.id}
      ...
      {payModal && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-fade-in">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => { setPayModal(null); setPayStep("form"); }} />
            ...
            {payStep === "success" ? (
              ...
              <>
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button onClick={() => setPayModal(null)} className="h-8 w-8 grid place-items-center rounded-lg hover:bg-foreground/5">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                ...
                <div className="rounded-xl bg-foreground/[0.03] p-4 mb-5">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground mt-0.5">{payModal.id} · Due: {payModal.due}</div>
                    </div>
                ...
                <div className="space-y-3">
                  ...
                  {[
                    ...
                    <label key={m.id} className="flex items-center gap-3 p-3 rounded-xl bg-foreground/[0.03] hover:bg-foreground/[0.06] cursor-pointer transition">
                      <input type="radio" name="method" defaultChecked={m.id === "upi"} className="accent-[oklch(from_var(--primary)_l_c_h)]" />
                      <div>
                ...
                <div className="mt-6 flex items-center justify-end gap-2">
                  <button onClick={() => setPayModal(null)} className="h-10 px-4 rounded-lg text-sm font-medium hover:bg-foreground/5">
                    Cancel
                  </button>
                  <button
                    onClick={handlePay}
