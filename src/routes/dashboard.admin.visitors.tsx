...
import { Search, X, LogIn, LogOut, Phone, Car, User } from "lucide-react";
...
type Visitor = {
  id: string; name: string; phone: string; type: "Guest" | "Delivery" | "Cab" | "Service";
  flat: string; host: string; checkIn: string; checkOut?: string; vehicle?: string;
};

const data: Visitor[] = [
  { id: "V-501", name: "Rohit Sharma", phone: "+91 99887 11223", type: "Guest", flat: "A-204", host: "Priya Mehta", checkIn: "09:42", vehicle: "MH-12 AB-1234" },
  { id: "V-500", name: "Amazon Delivery", phone: "+91 99887 22334", type: "Delivery", flat: "B-302", host: "Anika Sharma", checkIn: "09:31", checkOut: "09:38" },
  { id: "V-499", name: "Uber Cab", phone: "+91 99887 33445", type: "Cab", flat: "C-105", host: "Sunil Joshi", checkIn: "08:55", checkOut: "09:02", vehicle: "MH-04 XY-9988" },
  { id: "V-498", name: "Plumber - Suresh", phone: "+91 99887 44556", type: "Service", flat: "D-405", host: "Arjun Rao", checkIn: "08:20" },
  { id: "V-497", name: "Family Visit", phone: "+91 99887 55667", type: "Guest", flat: "A-101", host: "Ravi Kumar", checkIn: "07:15", checkOut: "08:55" },
];
...
function VisitorsPage() {
  ...
  const filtered = useMemo(() => data.filter((v) => {
    const matchQ = !q || v.name.toLowerCase().includes(q.toLowerCase()) || v.flat.toLowerCase().includes(q.toLowerCase());
    return matchQ && (type === "All" || v.type === type);
  }), [q, type]);

  const active = data.filter((v) => !v.checkOut);
  const tone = (t: Visitor["type"]) => t === "Guest" ? "primary" : t === "Delivery" ? "warning" : t === "Cab" ? "accent" : "success";
    ...
    <DashboardLayout role="Admin" items={adminNav}>
      <div className="space-y-6 animate-fade-up">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatCard label="Currently Inside" value={String(active.length)} icon={LogIn} tone="success" />
          <StatCard label="Deliveries" value={String(data.filter(v => v.type === "Delivery").length)} icon={Car} tone="warning" />
        ...
        <Card title={`Active Visitors (${active.length})`}>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {active.map((v) => (
              <button key={v.id} onClick={() => setSelected(v)} className="text-left rounded-xl glass p-4 hover:shadow-card transition">
        ...
        <Card title="Visitor Log" action={
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((v) => (
                  <tr key={v.id} onClick={() => setSelected(v)} className="border-b border-border last:border-0 hover:bg-foreground/[0.03] cursor-pointer">
                    <td className="px-2 py-3 font-medium">{v.id}</td>
                    <td className="px-2 py-3">{v.name}</td>
                    <td className="px-2 py-3"><Badge tone={tone(v.type)}>{v.type}</Badge></td>
                    ...
                    <td className="px-2 py-3 text-foreground/80">{v.checkOut ?? <span className="text-[color:var(--success)]">Inside</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-fade-in">
          ...
          <div className="relative w-full max-w-md rounded-2xl glass-strong shadow-elegant p-6 animate-scale-in">
            ...
            <div className="space-y-2.5 text-sm">
              ...
              {selected.checkOut && <Detail icon={LogOut} label="Checked out" value={selected.checkOut} />}
...
function Detail({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl bg-foreground/[0.03]">
      ...
      <div><div className="text-[11px] text-muted-foreground">{label}</div><div className="text-sm font-medium">{value}</div></div>
    </div>
  );
}
