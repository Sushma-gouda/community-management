...
import { Building2, Users, Layers, Maximize2, Calendar, Car, Wifi, Dumbbell, Waves, Shield } from "lucide-react";
      ...
      <div className="space-y-6 animate-fade-up">
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-4">
            <Card title="Family Members" action={<span className="text-xs text-muted-foreground">{familyMembers.length} members</span>}>
              <div className="space-y-2">
                {familyMembers.map((m) => {
                  const initials = m.name.split(" ").map((n) => n[0]).slice(0, 2).join("");
                  return (
                    <div key={m.name} className="flex items-center gap-3 p-3 rounded-xl hover:bg-foreground/5 transition">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">{m.relation} · Age {m.age}</div>
                      </div>
                    </div>
                  );
                })}
