...
const notices = [
  { id: 1, title: "Water tank cleaning on Sunday", body: "There will be no water supply between 9 AM and 1 PM. Please store water in advance.", target: "All Blocks", date: "May 11", pinned: true, tag: "Important", tone: "warning" as const },
  { id: 2, title: "Fire drill scheduled - May 22", body: "Mandatory fire drill at 11 AM. All residents are requested to participate.", target: "All Blocks", date: "May 09", pinned: false, tag: "Safety", tone: "danger" as const },
  { id: 3, title: "New gym equipment arrived", body: "Treadmills and dumbbells installed in the community gym. Open from 6 AM to 10 PM.", target: "All Blocks", date: "May 05", pinned: false, tag: "Amenity", tone: "success" as const },
  { id: 4, title: "Block A lift maintenance", body: "Lift in Block A will be unavailable on May 18 from 10 AM to 4 PM.", target: "Block A", date: "May 03", pinned: false, tag: "Maintenance", tone: "primary" as const },
];
          ...
          <div className="lg:col-span-2">
            <Card title="Compose Notice" action={<Badge tone="primary">Draft</Badge>}>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Title"><TextInput placeholder="Write a clear title..." /></Field>
                <Field label="Tag"><SelectInput><option>Important</option><option>Safety</option><option>Maintenance</option><option>Amenity</option></SelectInput></Field>
              </div>
              <div className="mt-3">
                <Field label="Message">
                  <div className="rounded-lg bg-foreground/5 border border-transparent focus-within:border-input">
                    <div className="flex items-center gap-1 px-2 py-1.5 border-b border-border/50">
                      {[Bold, Italic, List, Link2].map((Icon, i) => (
                        <button key={i} className="h-7 w-7 grid place-items-center rounded-md hover:bg-foreground/5 text-foreground/70"><Icon className="h-3.5 w-3.5" /></button>
                      ))}
                    </div>
                    <textarea rows={6} placeholder="Type your announcement here..." className="w-full bg-transparent p-3 text-sm focus:outline-none resize-none" />
                  </div>
                </Field>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-3">
                <Field label="Target Audience">
                  <SelectInput><option>All Blocks</option><option>Block A</option><option>Block B</option><option>Block C</option><option>Block D</option></SelectInput>
                </Field>
                <Field label="Schedule"><TextInput type="datetime-local" /></Field>
              </div>
              <div className="mt-5 flex items-center justify-end gap-2">
                <button className="h-10 px-4 rounded-lg text-sm font-medium hover:bg-foreground/5">Save Draft</button>
                <PrimaryButton><Send className="h-4 w-4" /> Publish Notice</PrimaryButton>
              </div>
            </Card>
          </div>
