...
type Bill = {
  id: string; flat: string; resident: string; amount: number; due: string;
  status: "Paid" | "Pending" | "Overdue"; type: string;
};

const seed: Bill[] = [
  { id: "INV-9821", flat: "A-101", resident: "Ravi Kumar", amount: 4500, due: "May 30", status: "Paid", type: "Maintenance" },
  { id: "INV-9819", flat: "A-204", resident: "Priya Mehta", amount: 4500, due: "May 30", status: "Paid", type: "Maintenance" },
  { id: "INV-9817", flat: "B-302", resident: "Anika Sharma", amount: 6200, due: "Jun 5", status: "Pending", type: "Maintenance + Repair" },
  { id: "INV-9815", flat: "C-105", resident: "Sunil Joshi", amount: 4500, due: "May 15", status: "Overdue", type: "Maintenance" },
  { id: "INV-9813", flat: "C-204", resident: "Meera Pillai", amount: 4500, due: "Jun 5", status: "Pending", type: "Maintenance" },
  { id: "INV-9811", flat: "D-405", resident: "Arjun Rao", amount: 5200, due: "May 30", status: "Paid", type: "Maintenance" },
];
...
function BillingPage() {
  ...
  const filtered = useMemo(() => data.filter((b) => status === "All" || b.status === status), [data, status]);
  const tone = (s: Bill["status"]) => s === "Paid" ? "success" : s === "Pending" ? "warning" : "danger";
  ...
  return (
    <DashboardLayout role="Admin" items={adminNav}>
      <div className="space-y-6 animate-fade-up">
        <PageHeader title="Billing" subtitle="Generate, track, and reconcile maintenance bills."
          actions={<>
            <GhostButton><Download className="h-4 w-4" /> Export</GhostButton>
            <PrimaryButton onClick={() => setOpen(true)}><Plus className="h-4 w-4" /> Generate Bill</PrimaryButton>
          </>} />

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KPI label="Total Billed" value={`₹${totalAll.toLocaleString()}`} sub={`${data.length} invoices`} tone="primary" />
          <KPI label="Collected" value={`₹${totals.paid.toLocaleString()}`} sub={`${Math.round(totals.paid / totalAll * 100)}% of total`} tone="success" />
        ...
        <Card title="Invoices">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((b) => (
                  <tr key={b.id} className="border-b border-border last:border-0 hover:bg-foreground/[0.02]">
                    <td className="px-2 py-3 font-medium flex items-center gap-2">
                      <span className="grid place-items-center h-7 w-7 rounded-md bg-primary/10 text-primary"><FileText className="h-3.5 w-3.5" /></span>
                      {b.id}
                    </td>
                    ...
                    <td className="px-2 py-3"><DueIndicator due={b.due} status={b.status} /></td>
                    <td className="px-2 py-3"><Badge tone={tone(b.status)}>{b.status}</Badge></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Generate Bill" footer={
        <>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Bill Type"><SelectInput><option>Maintenance</option><option>Repair</option><option>Penalty</option></SelectInput></Field>
          <Field label="Cycle"><SelectInput><option>Monthly</option><option>Quarterly</option></SelectInput></Field>
          <Field label="Target"><SelectInput><option>All Flats</option><option>Block A</option><option>Block B</option></SelectInput></Field>
          <Field label="Due Date"><TextInput type="date" /></Field>
          <div className="col-span-2"><Field label="Amount per Flat (₹)"><TextInput type="number" defaultValue={4500} /></Field></div>
          <div className="col-span-2"><Field label="Notes"><TextInput placeholder="Optional remarks" /></Field></div>
        </div>
      </Modal>
    </DashboardLayout>
  );
      ...
      <div className="flex-1 space-y-2">
        {segs.map((s) => (
          <div key={s.l} className="flex items-center justify-between text-xs">
            <span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-sm" style={{ background: `oklch(from ${s.c} l c h)` }} /> {s.l}</span>
