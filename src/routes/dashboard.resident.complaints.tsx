...
type Complaint = {
  ...
  status: Status; priority: Priority; created: string; updated: string;
  timeline: { label: string; date: string; done: boolean }[];
};

const seed: Complaint[] = [
  {
    id: "C-2041", title: "Lift not working in Block B", desc: "The lift between floors 1 and 5 has been stuck since morning. Elderly residents are facing difficulty.",
    category: "Elevator", status: "Open", priority: "Critical", created: "May 9, 2026", updated: "2 hours ago",
    timeline: [
  ...
  {
    id: "C-2018", title: "AC drainage issue in bedroom", desc: "Water is dripping from the AC unit in the master bedroom. The wall is getting damp.",
    category: "Plumbing", status: "In Progress", priority: "High", created: "May 3, 2026", updated: "1 day ago",
    timeline: [
  ...
  {
    id: "C-1996", title: "Intercom not working", desc: "The intercom unit in the flat is not receiving calls from the gate.",
    category: "Electrical", status: "Resolved", priority: "Medium", created: "Apr 22, 2026", updated: "Apr 25, 2026",
    timeline: [
...
const categories = ["Elevator", "Plumbing", "Electrical", "Housekeeping", "Noise", "Security", "Other"];
...
function ResidentComplaints() {
  ...
  const [form, setForm] = useState({ title: "", category: "Elevator", desc: "", priority: "Medium" as Priority });
  ...
  const StatusIcon = ({ s }: { s: Status }) =>
    s === "Resolved" ? <CheckCircle2 className="h-4 w-4 text-[color:var(--success)]" /> :
    s === "In Progress" ? <Clock className="h-4 w-4 text-primary" /> :
    ...
    <DashboardLayout role="Resident" items={residentNav}>
      <div className="space-y-6 animate-fade-up">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Open", value: data.filter((c) => c.status === "Open").length, color: "var(--warning)" },
            { label: "In Progress", value: data.filter((c) => c.status === "In Progress").length, color: "var(--primary)" },
            { label: "Resolved", value: data.filter((c) => c.status === "Resolved").length, color: "var(--success)" },
          ].map((s) => (
            <div key={s.label} className="rounded-2xl glass shadow-card p-5">
              <div className="text-xs text-muted-foreground">{s.label}</div>
              <div className="mt-2 text-2xl font-semibold" style={{ color: `oklch(from ${s.color} l c h)` }}>
                {s.value}
              </div>
            </div>
          ))}
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((c) => (
            ...
            >
              ...
              <h3 className="font-semibold text-base group-hover:text-primary transition line-clamp-2">{c.title}</h3>
              ...
              <div className="mt-4 flex items-center gap-1">
                {c.timeline.map((t, i) => (
                  <div key={i} className="flex items-center gap-1 flex-1">
                    <div className={`h-2 w-2 rounded-full shrink-0 ${t.done ? "bg-[image:var(--gradient-primary)]" : "bg-foreground/15"}`} />
                    {i < c.timeline.length - 1 && (
                      <div className={`flex-1 h-0.5 ${t.done ? "bg-primary/40" : "bg-foreground/10"}`} />
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-fade-in">
          ...
          <div className="relative w-full max-w-xl rounded-2xl glass-strong shadow-elegant p-6 animate-scale-in max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              ...
              <button onClick={() => setSelected(null)} className="h-8 w-8 grid place-items-center rounded-lg hover:bg-foreground/5 shrink-0">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="ml-auto text-xs text-muted-foreground">Raised: {selected.created}</span>
            </div>
              ...
              <ol className="relative ml-3 space-y-4">
                {selected.timeline.map((t, i) => (
                  <li key={i} className="pl-6 relative">
                    <span className={`absolute left-0 top-1 h-3 w-3 rounded-full border-2 ${t.done ? "bg-[image:var(--gradient-primary)] border-primary" : "bg-background border-border"}`} />
                    {i < selected.timeline.length - 1 && (
                      <span className={`absolute left-[5px] top-4 bottom-[-1rem] w-0.5 ${t.done ? "bg-primary/40" : "bg-border"}`} />
                    )}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-fade-in">
          ...
          <div className="relative w-full max-w-lg rounded-2xl glass-strong shadow-elegant p-6 animate-scale-in">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowForm(false)} className="h-8 w-8 grid place-items-center rounded-lg hover:bg-foreground/5">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="mt-6 flex items-center justify-end gap-2">
              <button onClick={() => setShowForm(false)} className="h-10 px-4 rounded-lg text-sm font-medium hover:bg-foreground/5">
                Cancel
              </button>
              <button
                onClick={submit}
